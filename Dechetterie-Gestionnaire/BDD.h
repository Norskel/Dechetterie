...
public:
	...
	List<DataHistorique^>^ getTableHistoriqueByIdUser(int id_User)
	{
		...
		{
			List<DataHistorique^>^ t = gcnew List<DataHistorique^>;
			MySqlCommand^ request = gcnew MySqlCommand("call Dechetterie.RechercheUtilisateur(@idUser);", _con);
	...
	void updateUser(DataUser^ e) override
	{
		if (ping())
		{
			...
			MySqlCommand^ request = gcnew MySqlCommand("call Dechetterie.EditUtilisateur(@nom, @prenom, @ID_RFID, @ville, @adresse, @numero, @codePostal);", _con);
	...
	void deleteUserByID(int id) override
	{
		if (ping())
		{
			MySqlCommand^ request = gcnew MySqlCommand("DELETE FROM `Dechetterie`.`Utilisateur` WHERE `ID`=@ID;", _con);
